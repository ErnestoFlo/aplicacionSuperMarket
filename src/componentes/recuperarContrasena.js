...
import ButtonGradient from './buttonRecuperar';
...
export default function App() {
  ...
  const preRecuperarContrasena = async () =>{
    console.log(correo);
    if (!correo) {
      ...
      try {
        const respuesta = await fetch(
          'http://192.168.0.3:4001/api/autenticacion/recuperarcontrasena', {
            method: 'POST',
            headers: {
  ...
  return (
    ...
    <View style={styles.container}>
      ...
      <Text style={styles.subtitulo}>Envie su correo electronico</Text>
      <TextInput style={styles.textInput} value={correo} onChangeText={setCorreo} placeholder='Juanito123@gmail.com.'/>
      <ButtonGradient onPress={preRecuperarContrasena}/>
      
      <StatusBar style="auto" />
      <Button onPress={preRecuperarContrasena}  title={"                                    "} color='rgba(69,69,69,0)' style={styles.prueba}></Button>
    </View>
    </View>
  );
}
